...
/**
 * Tính giá bán lẻ Resell (Markup) và làm tròn theo tâm lý học giá (Charm Pricing)
 * Nguyên tắc mốc thấp nhất:
 * - Dưới 50.000đ: Kê 80% (làm tròn lên đuôi 9.000đ, VD: 15k -> 29k, 20k -> 39k)
 * - 50.000đ - 200.000đ: Kê 35% (làm tròn lên đuôi 9.000đ, VD: 50k -> 69k, 100k -> 139k)
 * - 200.000đ - 500.000đ: Kê 20% (làm tròn lên đuôi 9.000đ, VD: 220k -> 269k, 350k -> 429k)
 * - Trên 500.000đ: Kê 15% (làm tròn lên 10.000đ / 50.000đ, VD: 750k -> 869k, 1.25tr -> 1.45tr)
 */
export function calculateResellPrice(costPrice) {
  const cost = Number(costPrice) || 0
  if (cost <= 1000) return cost // Giữ nguyên các item test / tượng trưng như 1đ, 550đ
  ...
  // 1. Dưới 10.000đ: Làm tròn lên nghìn (VD: 5k * 1.8 = 9.000đ)
  if (raw < 10000) {
    return Math.ceil(raw / 1000) * 1000
  }

  // 2. Từ 10.000đ đến 1.000.000đ: Làm tròn lên mốc đuôi 9.000đ gần nhất (VD: 29k, 49k, 69k, 89k, 139k, 269k, 429k, 869k)
  if (raw < 1000000) {
    const step = 10000
    return Math.ceil((raw - 9000) / step) * step + 9000
  }

  // 3. Từ 1.000.000đ đến 3.000.000đ: Làm tròn lên bội số 10.000đ gần nhất
  if (raw < 3000000) {
    const step = 10000
    return Math.ceil(raw / step) * step
  }

  // 4. Trên 3.000.000đ: Làm tròn lên bội số 50.000đ gần nhất cho chẵn đẹp
  const step = 50000
  return Math.ceil(raw / step) * step
}
