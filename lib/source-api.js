...
function requireEnv() {
  if (!BASE_URL || !API_KEY) {
    throw new Error(
      '[source-api] SOURCE_API_BASE and SOURCE_API_KEY environment variables are required. ' +
      'Add them to .env.local — never hardcode credentials in source files.'
    )
  }
}
...
/**
 * Normalize raw product from API to standard shop schema
 */
function normalizeProduct(p, override = null) {
  ...
  const hasCustomPrice = override && Number(override.sell_price) > 0
  const sellPrice = hasCustomPrice ? Number(override.sell_price) : autoPrice
...
export async function getCatalog() {
  ...
  // Fetch overrides from Supabase product_cache
  ...
  try {
    ...
    const { data: cacheRows, error: cacheErr } = await serviceClient
      .from('product_cache')
      .select('*')
    if (!cacheErr && Array.isArray(cacheRows)) {
      cacheRows.forEach((row) => overridesMap.set(row.id, row))
    }
  } catch (err) {
    console.warn('[source-api] Could not load product_cache overrides:', err.message)
  }

  // Filter hidden products and apply custom price overrides
  const visibleProducts = activeProducts.filter((p) => {
    const id = p.productId || p.id
    const override = overridesMap.get(id)
    if (override && override.is_visible === false) {
      return false
    }
    return true
  })

  const normalized = visibleProducts.map((p) => {
    const id = p.productId || p.id
    const override = overridesMap.get(id)
    return normalizeProduct(p, override)
  })
